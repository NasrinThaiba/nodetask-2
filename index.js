...
app.get('/customer/:name', (req, res) => {
  ...
  const customerBookings = customer.bookings.map(booking => {
    ...
    return { roomId, startTime, endTime, bookingID, status, bookingDate, booked_On };
  });

  const bookingCount = customerBookings.length;

  res.json({ customer: name, bookingCount, bookings: customerBookings });
});



app.listen(port, ()=> console.log("Hallbooking server is running in the port", port));